...
const start = (client = new Client()) => {
  ...
  client.onMessage((message) => {
    ...
    client
      ...
      .then((msg) => msg >= 3000 && client.cutMsgCache());
    // Message Handler
  ...
  client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
    client.getGroupMembersId(id).then((ids) => {
      console.log(
        "[CLIENT]",
        color(`Invited to Group. [ ${name} => ${ids.length}]`, "yellow")
      );
      ...
      if (ids.length <= 10) {
        client
          .sendText(
            id,
            "Sorry, the minimum group member is 10 user to use this bot. Bye~"
          )
          .then(() => client.leaveGroup(id));
      } else {
        client.sendText(
          id,
          `Hello group members *${name}*, thank you for inviting this bot, to see the bot menu send *#menu*`
        );
      }
    })
  );
